tag = "Wighted_constant_LinearSVC_NoProjection_NoNoise_NoNorm_MovedToGPU"
...
class Classifier(torch.nn.Module):
    def __init__(
        ...
        self.svm = SVC(kernel=kernel, C=C, probability=False, cache_size=20_000)
    ...
    def train_svm(self, train_loader):
        ...
        with torch.no_grad():
            for i, batch in enumerate(train_loader, 0):
                ...
                noises = [
                    torch.randn_like(inputs, device=device) * sigma for _ in range(10)
                ]

                inputs_c = torch.cat([inputs + noise for noise in noises], dim=0)
                labels_c = torch.cat([labels for _ in range(10)], dim=0)

                inputs_c = self.normalizer(inputs_c)
                inputs_c = self.encoder(inputs_c)
                inputs_c = F.normalize(inputs_c, dim=1)

                # train_weights.append(
                #     torch.linalg.norm(noise.view(noise.shape[0], -1), dim=1)
                #     .cpu()
                #     .numpy()
                # )
                train_weights.append(torch.ones(inputs_c.shape[0]).cpu().numpy())
                train_features.append(inputs_c.cpu().numpy())
                train_labels.append(labels_c.cpu().numpy())
        ...
        self.svm.fit(train_features, train_labels, sample_weight=train_weights)
        ...
        print(self.linear.weight.data.shape, self.linear.bias.data.shape)
    ...
    def _ovr_decision_function(self, predictions, confidences, n_classes):
        ...
        votes = torch.zeros(n_samples, n_classes).to(predictions.device)
        sum_of_confidences = torch.zeros(n_samples, n_classes).to(predictions.device)
        ...
        transformed_confidences = sum_of_confidences / (
            3 * (torch.abs(sum_of_confidences) + 1)
        )
    ...
    def forward(self, x):
        ...
        return self._ovr_decision_function(x < 0, -x, 10)
