...
class Classifier(torch.nn.Module):
    def __init__(
        ...
        self.predictor = torch.nn.Linear(128, 10).to(device)
    ...
    def train_svm(self, train_loader):
        ...
        self.predictor.weight.data = torch.from_numpy(self.svm.coef_).to(device).float()
        self.predictor.bias.data = (
            torch.from_numpy(self.svm.intercept_).to(device).float()
        )

        # self.svm = self.linear
...
optimizer = optim.Adam(classifier.predictor.parameters(), lr=freeze_lr)
...
exp_name = "{}_reduced_svm_C_{}_lbd_{}_eta_{}".format(tag, args.C, args.lbd, args.eta)
