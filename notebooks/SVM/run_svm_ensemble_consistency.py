...
weights_file = args.model_consistency_path
...
class SVM_Classifier(torch.nn.Module):
    def __init__(
        self,
        encoder_weights,
        C=1.0,
...
classifier1 = SVM_Classifier(
    args.model_svm_path1,
    C=args.C,
).to(device)

classifier2 = SVM_Classifier(
    args.model_svm_path2,
    C=args.C,
).to(device)
...
class Ensemble(torch.nn.Module):
    ...
    def forward(self, x):
        x1 = F.softmax(self.classifier1(x), dim=1)
        x2 = F.softmax(self.classifier2(x), dim=1)
        x3 = F.softmax(self.classifier3(x), dim=1)

        return torch.max(x1, torch.max(x2, x3))


final_classifier = Ensemble(classifier1, classifier2, classifier).to(device)
final_classifier = torch.nn.DataParallel(final_classifier).to(device)
...
smoothed_classifier = Smooth(final_classifier, 10, sigma)

# prepare output file

exp_name = "{}_reduced_svm_{}".format(tag, args.C)
