...
TASK_ID = int(os.environ["SLURM_LOCALID"]) + 8
...
test_loader = torch.utils.data.DataLoader(
    test_dataset, batch_size=1000, shuffle=False, num_workers=6
)
train_loader = torch.utils.data.DataLoader(
    train_dataset, batch_size=512, shuffle=True, num_workers=6
)
...
weights_file = f"{base_path}ckpt_epoch_{100 * (TASK_ID + 1)}.pth"
...
def print_classifier_params():
    print(
        f"[{TASK_ID}]: Number of parameters in the Encoder: {sum([p.numel() for p in classifier.encoder.parameters()]) / 1000_000}"
    )
    print(
        f"[{TASK_ID}]: Number of parameters in the Predictor: {sum([p.numel() for p in classifier.predictor.parameters()]) / 1000_000}"
    )
    print(
        f"[{TASK_ID}]: Number of parameters in the Classifier: {sum([p.numel() for p in classifier.parameters()]) / 1000_000}"
    )
    print(
        f"[{TASK_ID}]: Number of trainable parameters in the Classifier: {sum([p.numel() for p in classifier.parameters() if p.requires_grad]) / 1000_000}"
    )
...
def train(n_epochs, model, optimizer, criterion, scheduler):
    for epoch in range(n_epochs):  # loop over the dataset multiple times
        ...
        print(
            f"[{TASK_ID}]: Accuracy of the network on the 10000 test images: {100 * correct / total}%"
        )
        ...
        print(
            f"[{TASK_ID}]: Accuracy of the network on the 10000 test images: {100 * correct / total}% | Using noise with sigma={sigma}"
        )
...
optimizer = optim.Adam(classifier.parameters(), lr=unfreeze_lr)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=8, eta_min=0.01)
...
exp_name = "{}_consistency_finetuned_reduced{}".format(100 * (TASK_ID + 1), arch_name)
...
for i, batched_sample in enumerate(test_dataset):
    ...
    if i % 100 == 0:
        print(f"[{TASK_ID}] {i} / {len(test_dataset)}")
