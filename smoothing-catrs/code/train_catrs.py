...
parser.add_argument(
    "--id", default=None, type=int, help="experiment id, `randint(10000)` if None"
)
...
parser.add_argument(
    "--pretrained-model", type=str, default="", help="Path to a pretrained model"
)
...
parser.add_argument(
    "--lbd", default=1.0, type=float, help="strength of the contribution of L^high"
)
parser.add_argument(
    "--confidence_mask",
    action="store_true",
...
parser.add_argument(
    "--lr_drop", default=10000, type=int, help="drops learning rate for given epoch"
)
parser.add_argument(
    "--warmup",
    default=10000,
...
# Based on batch size, adjust learning rate
args.lr = args.lr * args.batch / 256
...
def main():
    ...
    for epoch in range(starting_epoch, args.epochs):
        ...
        if epoch >= args.warmup:
            attacker = KL_PGD(
                steps=args.num_steps, device=device, max_norm=args.epsilon * 2.0
            )
...
def train(
    ...
    for i, batch in enumerate(loader):
        ...
        for inputs, targets, pb in mini_batches:
            ...
            t_sm0 = [
                F.one_hot(torch.argmax(logit, dim=1), n_classes)
                for logit in logits0_chunk
            ]
            ...
            loss_xent = [
                F.cross_entropy(logit, targets, reduction="none").view(-1, 1)
                for logit in logits0_chunk
            ]
            ...
            loss_kl = [
                F.kl_div(F.log_softmax(logit, dim=1), pb, reduction="none").sum(
                    1, keepdim=True
                )
...
class KL_PGD(object):
    ...
    def attack(self, model, inputs, labels, noises=None):
        ...
        for i in range(self.steps):
            ...
            loss_kls = [
                F.kl_div(F.log_softmax(logit, dim=1), labels, reduction="none").sum(1)
                for logit in logits_chunk
            ]
            ...
            sigma = (
                (noise**2).view(inputs_r.size(0), -1).mean(1).sqrt().view(-1, 1, 1, 1)
            )
