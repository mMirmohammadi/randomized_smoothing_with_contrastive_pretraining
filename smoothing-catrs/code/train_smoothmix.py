...
parser.add_argument(
    "--alpha", default=1.0, type=float, help="step-size for adversarial attacks."
)
parser.add_argument(
    "--num-steps",
    default=8,
...
def main():
    ...
    attacker = SmoothMix_PGD(
        ...
        mix_step=args.mix_step,
        alpha=args.alpha,
        maxnorm=args.maxnorm,
        maxnorm_s=args.maxnorm_s,
    )
...
def train(
    ...
    for i, batch in enumerate(loader):
        ...
        for inputs, targets in mini_batches:
            ...
            noises = [
                torch.randn_like(inputs) * noise_sd for _ in range(args.num_noise_vec)
            ]
            ...
            in_mix, targets_mix = _mixup_data(
                inputs, inputs_adv, clean_avg_sm, args.n_classes
            )
            ...
            loss = (
                loss_xent.mean()
                + args.eta * args.warmup_v * (ind_correct * loss_mixup).mean()
            )
