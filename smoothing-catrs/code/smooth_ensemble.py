...
class SmoothEnsemble(object):
    ...
    def certify(self, x: int, n0: int, n: int, alpha: float, batch_size: int) -> (int, float):
    ...
    def _sample_noises(self, x: int, num: int, batch_size) -> np.ndarray:
        outputs = [base_classifier[x,:num] for base_classifier in self.base_classifiers]
        predictions = self._get_predictions(outputs, num)
        counts = np.zeros((self.num_classifiers*2, self.num_classes), dtype=int)
        for i, prediction in enumerate(predictions):
            counts[i] += self._count_arr(prediction.numpy(), self.num_classes)
        return counts
    ...
    def _get_predictions(self, outputs, batch_size) -> List[torch.Tensor]:
        ...
        for i, output in enumerate(outputs): # for each base classifier
            if self.aggregation_scheme == 0: # soft-voting without softmax - default
                ...
                ensemble_outputs += torch.nn.functional.softmax(output, 0)
            ...
            if self.aggregation_scheme == 1: # if draw after hard voting, use randomness
                ensemble_outputs = ensemble_outputs + torch.rand(batch_size, self.num_classes) * 0.1
